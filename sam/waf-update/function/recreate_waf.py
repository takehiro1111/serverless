...
def lambda_handler(event, context):
    ...
    logger.info(
        f"Attempting to delete rule {recreate_rule_name} from WebACL {web_acl_name}"
    )
    ...
    try:
        ...
        logger.info(
            f"Rule {recreate_rule_name} recreated successfully from WebACL {web_acl_name}"
        )
        ...
        send_slack_notification(
            "Failed", account, account_id, web_acl_name, "Recreate", ":x:"
        )
        raise e
