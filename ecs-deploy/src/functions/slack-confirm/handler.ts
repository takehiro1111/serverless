...
async function slackConfirm(event, context) {
  ...
  if (ecsDeployParams['auto-deploy'].BOOL) {
    ...
    await axios.post(
      'https://api.github.com/repos/nextbeat-dev/nextbeat-actions/actions/workflows/ecs-deploy.yml/dispatches',
      {
        ref:    'main',
        inputs: {
