...
```bash
test-resource$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
test-resource$ sam local start-api
test-resource$ curl http://localhost:3000/
```
...
```bash
test-resource$ sam logs -n HelloWorldFunction --stack-name test-resource --tail
```
...
```bash
sam delete --stack-name test-resource
```
