...
class NetworkStack(cdk.Stack):
    ...
    def create_igw(self) -> ec2.CfnInternetGateway:
        igw = ec2.CfnInternetGateway(
            self, "cdk_igw", tags=[{"key": "Name", "value": "cdk-igw"}]
        )
        ec2.CfnVPCGatewayAttachment(
            self, "cdk_igw_attach", vpc_id=self.vpc.ref, internet_gateway_id=igw.ref
        )
        return igw
    ...
    def create_eip_nat(self) -> ec2.CfnEIP:
        eip_nat = ec2.CfnEIP(self, "cdk_nat_a_eip", domain="vpc")
        return eip_nat
    ...
    def create_route_tables(self) -> dict:
        ...
        for name, (rtb_name, gateway_id) in rtb_configs.items():
            rtb[name] = ec2.CfnRouteTable(
                self,
                rtb_name,
                vpc_id=self.vpc.ref,
                tags=[{"key": "Name", "value": f"{rtb_name}-test"}],
            )
            if gateway_id:
                ec2.CfnRoute(
                    self,
        ...
        for rtb_name, subnet_name in associations:
            ec2.CfnSubnetRouteTableAssociation(
                self,
                f"{subnet_name}_route_associate",
